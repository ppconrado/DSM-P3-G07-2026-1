...
async function validateSessionDateRange(eventId, sessionDate) {
  ...
  const sessionKey = toDateOnlyKey(sessionDate);
  const startKey = toDateOnlyKey(event.startDate);
  const endKey = toDateOnlyKey(event.endDate);
...
export async function createEventSession(req, res) {
  try {
    ...
    await validateSessionDateRange(req.params.eventId, sessionDate);
...
export async function updateEventSession(req, res) {
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(data, 'sessionDate')) {
      ...
      await validateSessionDateRange(req.params.eventId, data.sessionDate);
    }
